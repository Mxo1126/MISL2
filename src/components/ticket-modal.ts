...
export class TicketModal {
    ...
    private selectedTicketType: string = '';
    ...
    public async open(gameId: string): Promise<void> {
        this.gameId = gameId;
    ...
    private updateTicketTypes(tickets: any[]): void {
    ...
    private updateQuantity(change: number): void {
        const input = this.modal.querySelector('#ticket-quantity') as HTMLInputElement;
        if (!input) return;
    ...
    private updateTotal(): void {
        ...
        if (!ticketType || !totalElement) return;
    ...
    private updatePaymentDetails(e: Event): void {
    ...
    private getPaymentDetails(formData: FormData): any {
